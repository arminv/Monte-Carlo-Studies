...
curve(3*x^2, xlim=c(-3,3), ylim=c(0,20), col="red", main="Example 1 : Importance Density g(x) & Function f(x)")
# f, the function
curve(3*x^4 + 2*x*exp(x) + 1 , xlim=c(-3,3), ylim=c(0,20), col="blue", add=T)
# upper bound of the integral
abline(v=1, col="green")

# 1) According to a rule of thumb, when the function f is large importance density g has to be large too. 
# In other words the candidate density g(x) has to be approximately proportional to the function f(x). 
# This rule holds in the case of g(x) = 3*x^2 and this is desirable.
...
boxplot(resA1, resB1, resC1, resD1, names=c("No Stratification", "Equal Number", "Proportional, 10", "Proportional, 30"), 
...
boxplot(res1_1, res2_1, res3_1, res4_1, res5_1, names=c("Basic MC", "Importance MC", "Strat A", "Strat B", "Strat C"),
        ylim=c(3.45,3.7), col=grey(0.8), range=0, main = "Example 1 : All Methods Compared")
abline(h=3.6, col="red", lwd=1)

# Conclusion: The stratified with proportional allocation (optimal) from each stratum and 30 strata is the best method. 
...
curve(4*x^3, xlim=c(-3,3), ylim=c(0,20), col="red", main="Example 2 : Importance Density g(x) & Function f(x)")
# f, the function:
curve(3*x^4 + 2*x*exp(x) + 1 , xlim=c(-3,3), ylim=c(0,20), col="blue", add=T)
# upper bound of the integral:
abline(v=1, col="green")

# According to a rule of thumb, when the function f is large importance density g has to be large too. 
# In other words the candidate density g(x) has to be approximately proportional to the function f(x). 
# This rule holds in the case of g(x) =  4*x^3 and this is desirable.
...
boxplot(res1_2, res2_2, res3_2, res4_2, res5_2, names=c("Basic MC", "Importance MC", "Strat A", "Strat B", "Strat C"), 
        ylim=c(3.45,3.7), col=grey(0.8), range=0, main = "Example 2 : All Methods Compared")
abline(h=3.6, col="red", lwd=1)

# Conclusion: In this case too, the stratified with proportional allocation (optimal) from each stratum and 30 strata is
# the best method. It has the closest estimate to the true value of the integral as well as having the smallest error margin. 
# Next are the other stratified sampling methods followed by simple monte carlo and importance sampling. 
